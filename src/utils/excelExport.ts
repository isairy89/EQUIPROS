...
export function exportConducesToExcel(conduces: Conduce[], fileName: string = 'Conduces_EQUIPROCI.xlsx') {
  const data = conduces.map((c) => ({
    ...
    'Tipo': c.tipoConduce === 'EQUIPO' ? 'Equipo Pesado' : 'Material / Volteo',
    'Fecha': c.fecha,
    'Hora': c.hora || '',
    ...
    'Operador / Chofer': c.choferNombre || '',
    'Equipo': c.equipoFicha || '',
    'Placa': c.placa || '',
    'Turno': c.turnoHorario || '',
    'Hora Inicial': c.horaInicio || '',
    'Hora Final': c.horaFin || '',
    'Horas Trabajadas': c.horasTrabajadas ?? '',
    'Viajes': c.viajes ?? '',
